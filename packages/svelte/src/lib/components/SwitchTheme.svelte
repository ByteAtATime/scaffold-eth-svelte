<script lang="ts">
  ...
  $effect(() => {
    document.body.setAttribute("data-theme", darkMode?.isDarkMode ? "dark" : "light");
  });
...
<div class={`flex space-x-2 text-sm ${className}`}>
  ...
  />
  <label for="theme-toggle" class="swap swap-rotate" class:swap-active={darkMode?.isDarkMode}>
